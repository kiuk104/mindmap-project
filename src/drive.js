 ...
 *
 * 사용 흐름:
 *   1. initDrive()        앱 시작 시 호출 (스크립트 로드 + 토큰 클라이언트 준비)
 *   2. signIn()           사용자가 "Drive 연결" 클릭 시 — 팝업 OAuth
 *   3. saveToDrive()      현재 마인드맵을 같은 이름 파일로 저장 (overwrite)
 *   4. listMindmaps()     JSON 파일 목록
 *   5. loadFromDrive()    파일 ID로 JSON 내용 가져오기
 *
 * scope: drive.file — 이 앱이 만든 파일만 보고/수정. 다른 파일 접근 불가.
 */
...
/** 초기화 — GIS + gapi.client.drive 로드. config 미설정 시 즉시 false 반환 */
 ...
 * @param {string} jsonContent - 직렬화된 JSON 문자열
